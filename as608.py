...
class Operation:
    ...
    def read_sysparam(self):
        ...
        # fig_print_log(bytes(r[1:3]) ,bytes(r[3:5]))
    ...
    def _get_packet(self, expected):
        ...
        if (not res) or (len(res) != expected):
            raise RuntimeError("Failed to read data from sensor -g, expected len %d, got %d" % (expected, len(res)))
        ...
        # fig_print_log(packet_sum)
        # fig_print_log(packet_type + length + struct.unpack('>HHHH', res[9:9+(length-2)]))
    ...
    def _print_debug(self, info, data, data_type="str"):
        ...
        if data_type == "hex":
            fig_print_log("*** DEBUG ==>", info, ["{:02x}".format(i) for i in data])
        elif data_type == "str":
            fig_print_log("*** DEBUG ==>", info, data)
...
def get_templates_list(session):
    ...
    fig_print_log("Fingerprint templates: ", session.templates)
...
def get_templates_count(session):
    ...
    fig_print_log("Number of templates found: ", session.template_count)
...
def get_device_size(session):
    ...
    fig_print_log("Size of template library: ", session.library_size)
...
def enroll_save_to_file(session, as608_lib, save_location, file_name):
    ...
    fig_print_log("Downloading template...")
    ...
    file.close()
    fig_print_log("Template is saved in", saved_directory_file_name, "file.")
...
def fingerprint_check_one_file(session, as608_lib, save_location, file_name):
    ...
    fig_print_log("Loading file template...", end="", flush=True)
    try:
        with open(search_directory_file_name, "rb") as file:
            data = file.read()
    except FileNotFoundError as e:
        fig_print_log("Could not find file.")
    else:
        with open(search_directory_file_name, "rb") as file:
            data = file.read()
    ...
    if i == as608_lib.OK:
        fig_print_log("Fingerprint match template in file.")
        return True
    if i == as608_lib.NOMATCH:
        fig_print_log("Templates do not match!")
    else:
        fig_print_log("Other error!")
    return False
...
def fingerprint_check_all_file(session, as608_lib, save_location):
    ...
    if os.path.exists(save_location):
        # fig_print_log(os.listdir(save_location))
        for each_file in os.listdir(save_location):
            # fig_print_log("=-=-=-ENtering file-=-=-=-=--=",each_file)
