...
def doorOpen():
    ...
    pwm = PWM(pinMotor, freq=gpioconfig.MOTOR_PWM_OPEN_FREQ, duty=gpioconfig.MOTOR_PWM_OPEN_DUTY)
    motorPwmTimer.init(period=gpioconfig.MOTOR_ROLLTATE_DELAY, mode=Timer.ONE_SHOT, callback=lambda x: _doorTimerDeinit(x, pwm))
    pass
...
def doorClose():
    ...
    if isDoorOperating:
        return
    isDoorOperating = True
    pwm = PWM(pinMotor, freq=gpioconfig.MOTOR_PWM_CLOSE_FREQ, duty=gpioconfig.MOTOR_PWM_CLOSE_DUTY)
    motorPwmTimer.init(period=gpioconfig.MOTOR_ROLLTATE_DELAY, mode=Timer.ONE_SHOT, callback=lambda x: _doorTimerDeinit(x, pwm))
    pass
...
def doorOpenAndClose():
    ...
    isDoorOperating = True
    pwm = PWM(pinMotor, freq=gpioconfig.MOTOR_PWM_OPEN_FREQ, duty=gpioconfig.MOTOR_PWM_OPEN_DUTY)
    motorPwmTimer.init(period=gpioconfig.MOTOR_ROLLTATE_DELAY, mode=Timer.ONE_SHOT, callback=lambda x: _doorOpenAndClose2(x, pwm))
